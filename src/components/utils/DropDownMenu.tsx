...
const DropDownMenu: React.FC<DropDownMenuProps> = ({isOpen}) => {
    ...
    return(
        <AnimatePresence>
            <motion.div initial={{height: "0px", transition: {duration: 0.4, ease: "linear", delay: 0.2}}} animate={{height, opacity: 1, transition: {duration: 0.4, ease: "linear", delay: 0.1}}} className="w-full overflow-hidden fixed z-10 top-[10%] left-0 bg-secondary">
                    ...
                    <div className="flex flex-col">
                        {_pageRoutes_.map((_route, index) => (
                            <div className="py-3 text-center border-t border-t-amber-50 hover:bg-gray-200 hover:underline" key={index}>
                                <Link to={_route._path} className="capitalize text-[16.5px] font-[550]">{_route._name}</Link>
                            </div>
                        ))}
                    </div>
                </div>
